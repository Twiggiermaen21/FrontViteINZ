...
const BrowseCalendars = () => {
  ...
  const fetchCalendars = useCallback(async () => {
    ...
    try {
      ...
      if (err.response?.status === 401) setTimeout(() => window.location.reload(), 500);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="relative w-full max-w-[1512px] mx-auto bg-[#2a2b2b] rounded-4xl p-8 shadow-lg space-y-4">
      <h1 className="text-3xl font-bold text-white mb-4">Przeglądaj kalendarze</h1>
  ...
  return (
    <div key={key} className="w-full border rounded bg-white shadow p-2 flex flex-col items-center mb-2">
  ...
  .map((img) => (
    ...
    />
      ...
      {loading && (
        <p className="text-[#989c9e] text-center mt-4">Ładowanie kalendarzy...</p>
      )}
    </div>
  );
};
